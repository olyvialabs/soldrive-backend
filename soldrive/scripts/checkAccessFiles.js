  ...
  for (let signatureInfo of signatures) {
    try {
      ...
      if (transaction && transaction.meta && transaction.meta.logMessages) {
        ...
        const mintMessages = transaction.meta.logMessages.filter(log => log.includes("Token minted with mint address:"));
        ...
        if (mintMessages.length > 0 && metadataMessages.length > 0) {
          ...
          const logObject = {
            ...metadata,
            mintAddress: mintMessages[0].split(": ")[2],
          };
...
async function main() {
  ...
  const contractAddress = "CYLp1vYEbkebAtUqcLXkMfkTuAvvzGcPrkv8U5Dd5GNb";
  const allLogs = await getContractLogs(contractAddress);

  // User we want to know the balances of
  const walletAddress = "Dt66zsAwUb12H21cpVqRQx44kXFRfZxT3xCs2bKsY1XZ";
